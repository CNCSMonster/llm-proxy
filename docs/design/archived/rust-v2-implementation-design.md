# Rust v2 Implementation Design

Status: accepted (primary design authority for Rust v2)
Scope: Rust rewrite configuration model and implementation plan
Last updated: 2026-07-30

## 1. Goal

Rust v2 keeps the product goal of llm-proxy: expose stable local endpoints for coding agents while hiding upstream API keys and provider-specific protocol differences.
...
This document is the single Rust v2 design authority. Other Rust v2 draft design documents live under `docs/drafts/` for work-in-progress features; this document takes precedence when there is a conflict. Supporting non-draft sources remain useful as evidence only:

- [`../research/providers/rust-v2-current-provider-catalog.md`](../research/providers/rust-v2-current-provider-catalog.md) — provider-product research evidence backing catalog defaults.
...
Rust v2 keeps the Go/v1 credential ownership boundary while making the user-facing provider config more regular:
...
- Unlike Go/v1, Rust v2 stores all OAuth account material in **one unified OAuth store file**, not separate OpenAI and Antigravity files. The store contains multiple accounts keyed by a stable account key; by default that key is the provider ID.
...
The target Rust v2 user-facing schema uses model-level protocol-specific provider binding lists, while conversion details live in providers.
...
Rust v2 should separate the user-facing TOML decode shape from the runtime configuration shape:
...
The four thinking-related field groups live at two levels, because they describe two different things (evidence: provider research in `docs/research/providers/`):
...
These invariants should remain true as Rust v2 grows:
...
Rust v2 is a full local LLM gateway, not a limited proof of concept. This section defines the required product surface.

### 10.1 Supported local client protocols

Rust v2 must expose these client-facing protocol families:
...
**No compatibility routes**: Rust v2 uses only protocol-prefixed routes. There are no `/v1/...` compatibility routes. All clients must use the protocol-specific prefix (`/openai/v1/...`, `/responses/v1/...`, `/anthropic/v1/...`).
...
10. **Provider request frequency limiting / anti-ban protection**
   - cooldown and fingerprint blocking protect upstreams after failures, but correct requests can also overload or rate-limit a model-service product. Rust v2 therefore supports provider-scoped proactive request frequency limits in addition to reactive cooldowns.
   - Frequency limits live at the Rust v2 **provider** level because one provider represents a concrete model-service product/account/region in the new config model. All endpoints declared under that provider (`openai_chat`, `openai_responses`, `anthropic`, derived endpoints, etc.) share one frequency pool. This matches how product/account quota and anti-abuse systems are usually enforced upstream.
...
11. **Launch config generation**
   ...
   - Client-specific generated capability fields are projections from the core model metadata. Example: Claude Desktop `inferenceModels[].supports1m` is generated only from resolved model `context_window >= 1_000_000`; Rust v2 must not add a duplicate `supports_1m` config field or encode capability in a `[1M]` model-name suffix.

### 10.6 Provider research evidence

Provider/product defaults in this document are design placeholders unless backed by the current-product review in [`../research/providers/rust-v2-current-provider-catalog.md`](../research/providers/rust-v2-current-provider-catalog.md).
...
llm-proxy init [--config PATH]            # create fresh Rust v2 config; never migrates old schemas
...
Rust v2 has three core operator commands around provider/model configuration. All three must support both CLI/script operation and interactive TUI operation:
...
**Detailed design** (command matrix, flag semantics, TUI state machines, internal modules, and ChatGPT usage API) is in **§12**. Rust v2 uses subcommands for scriptable provider/model actions; bare `provider` and bare `model` are reserved for TUI entry in interactive terminals.
...
- Custom endpoint URLs are user input and should be validated as URLs, but Rust v2 should not invent a token-in-query masking model for built-in providers.
...
Rust v2 uses **ratatui + crossterm** as the TUI framework, following an Elm-inspired Model-Update-View architecture (the same pattern used by the Go v1 `bubbletea` library). This section specifies:
...
Rust v2 should use `toml_edit` for llm-proxy TOML updates so `init`, `connect`, `provider`, `model`, and future config editing preserve user comments, ordering, and unrelated fields. External JSON client configs should use `serde_json::Value` or typed structs only for fully managed projection files.

### 14.2 No migration command

Rust v2 does **not** provide `llm-proxy migrate`. The supported configuration lifecycle is:

1. `llm-proxy init` creates a fresh minimal Rust v2 config when no config exists.
...
Legacy Go/v1 configs are not rewritten in place by Rust v2. Users who need Rust v2 should create a new config with `init` and re-add providers/models through the management commands. This keeps the rewrite strict and avoids silently inventing provider endpoint declarations during schema changes.

### 14.3 Embedded docs

Rust v2 must preserve `llm-proxy doc` behavior:
...
Rust v2 should keep user data locations predictable and secure:
...
- OAuth account material stored in one unified typed store with restrictive file permissions; unlike Go/v1, Rust v2 must not split OpenAI and Antigravity OAuth material into separate files;
...
Rust v2 is complete for this design only when tests and E2E evidence cover the old feature set and the new CLI semantics.
...
This matters especially for Rust v2 because the endpoint declarations can make conversion look generic. The declarations are generic routing metadata; the conversion implementation is deliberately not generic.
...
| Rust v2 area | Same change unit? | Decision |
|---|---:|---|
...
Its old runtime rule is **not** carried forward: Rust v2 must not infer protocol support from mere provider presence. Protocol support is explicit:
...
This section is the executable development loop for AI agents implementing Rust v2. It keeps the workflow in the main implementation design so the design, acceptance criteria, and stop conditions live in one file.
...
|---|---|
| Design anchor | Section in `rust-v2-implementation-design.md` or a new draft section. |
...
## Design anchors
- rust-v2-implementation-design.md#...
- ADR-...
...
Goal: implement corrected current provider catalog for fresh Rust v2 configs and provider/model management.
...
- Stop if a client behavior changed upstream and current docs/research are missing.

#### M9 — Final release gate

Goal: prove Rust v2 satisfies this design document.
...
- an accepted ADR appears to conflict with a desired Rust v2 change.
...
Rust v2 uses three levels of done. Agents must name which level they are claiming.
...
| Rewrite done / release candidate | Rust v2 satisfies this design document except documented intentional future work. | M9 acceptance, full design audit, mock E2E, skipped-safe real-provider smoke plan. | Only explicitly documented post-release/future-work items. |
